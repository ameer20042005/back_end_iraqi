...
| الملف | الدور |
|---|---|
| `app/config.py` | مصدر إعدادات الموديل وRAG والمنافذ والمفاتيح |
...
| `app/context_blocks.py` | صياغة مراجع اللهجة والمواقع لمسار استخراج الطلب |
...
| `app/fallback.py` | نص الرد الاحتياطي المشترك بين واجهتي المحادثة |
...
لا يوجد أي كتالوج منتجات أو سجل طلبات مخزّن بهذا المستودع (`app/data/` غير موجود عمداً). واجهتا المحادثة تمرران طلبات الأدوات إلى العميل لينفذها، ومسار إنشاء الطلب يرسل الطلبات المكتملة إلى نظام خارجي:

- **البحث عن المنتجات**: العميل يرسل تعريف `search_products` ضمن طلب المحادثة، وينفذ الأداة في نظامه ثم يعيد النتيجة للموديل.
...
- **رابط باك اند السستم**: `system_backend_base_url` بـ`app/config.py` (افتراضياً `http://127.0.0.1:8081/internal`) — عدّل القيمة ومسار الإرسال في `app/order_gateway.py` عند توفر العقد الفعلي.
...
البنية: **FastAPI (منفذ 8000) عميل HTTP رفيع ← خادم vLLM OpenAI-متوافق (منفذ 18001)** يخدم الموديل المدموج بـ continuous batching حقيقي — مئات الطلبات المتزامنة تتقاسم الـ GPU تلقائياً (PagedAttention) بدون طوابير يدوية.
