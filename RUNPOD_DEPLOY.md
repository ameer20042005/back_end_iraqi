...
- **خانات مفاتيح API** (مبيعات/دعم/إنشاء طلبات) تحت مؤشر الحالة — معبّأة مسبقاً بنفس القيم الثابتة بـ`app/config.py`، وتُحفظ بمتصفحك (`localStorage`) فتقدر تعدّلها لو بدّلت المفاتيح بـ`.env`. كل نقطة بالقائمة ترسل تلقائياً هيدر `X-API-Key` بالمفتاح المطابق لخدمتها.
