...
الـ system prompt الكامل يعيش بملف plane.md **بنفس هذا المجلد** (المصدر الوحيد —
عدّله هناك)، ويُحقن معه مرجع جغرافي من قاعدة بيانات شركة التوصيل
(app/rag/locations.py) حتى لا يخطئ الموديل بالمحافظة (city).

ليش بجنب الكود مو بجذر المستودع ولا بـdocs/؟ plane.md أصل تشغيلي (يُقرأ وقت
الإقلاع) مو وثيقة. لما كان بالجذر انتقل ضمن ترتيب عام لملفات التوثيق إلى docs/
بلا تحديث المسار هنا، فانهار إقلاع التطبيق كله بـFileNotFoundError — لأن open()
تحت تنفَّذ وقت الاستيراد لا وقت الطلب، وapp/main.py يستورد هذا الملف عبر راوتري
order_intake وsales. وجوده بجنب قارئه يجعل المسار سطراً واحداً بلا ".." متسلسلة،
فينتقلان معاً بأي إعادة تنظيم قادمة.
"""
...
_PLANE_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "plane.md")
