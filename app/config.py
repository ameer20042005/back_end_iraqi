...
class Settings(BaseSettings):
    ...
    #
    # المقايضة محسوبة على A40 48GB بموديل 12B: الأوزان (bf16) تأخذ ~24.4GB من
    # أصل ~43.2GB متاحة، فما يتبقى للـ KV cache إلا ~17GB. وKV عند 12B يكلّف
    # ~0.375MB لكل توكن (48 طبقة × 8 رؤوس KV × head_dim 256 × 2 لـ K و V ×
    # bf16)، يعني كل زيادة بالسياق تُضرب بعدد الطلبات المتزامنة:
    #     4096  = 1.5GB/طلب  → ~11 طلباً متزامناً
    #     10000 = 3.7GB/طلب  → ~4  طلبات   ← الحالي
    #     20000 = 7.3GB/طلب  → ~2  طلبين   (يخنق التزامن ويطرد كاش البادئة)
    #
