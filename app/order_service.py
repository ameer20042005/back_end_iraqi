...
لا يحتفظ هذا المسار بكتالوج منتجات. قيمة المجموع المعادة تؤخذ من
quoted_price في نتيجة استخراج الطلب، ولا يعيد هذا الملف حساب أسعار المنتجات.
"""
...
def _new_order_id() -> str:
    ...
    وهي أسهل للزبون بالقراءة. الجزء العشوائي
    يبقى: ٦ خانات من uuid4 حتى يبقى المعرّف فريداً بلا عدّاد مشترك بين العمليات.

    **أرقام فقط** لا خانات سداسية عشرية حتى يكون المعرّف سهلاً على الزبون
    بالقراءة والنقل بالهاتف. تسع خانات: احتمال التصادم مهمل بحجم الطلبات المتوقَّع.
...
def _submission_blockers(order: OrderConfirmation) -> List[str]:
    ...
    ليش موجود: بوابة العميل تحرس **المدخل**
